...
#define TAS5806MD_RATES		(SNDRV_PCM_RATE_48000)
#define TAS5806MD_FORMATS	(SNDRV_PCM_FMTBIT_S32_LE)
...
static void tas5806md_refresh(struct snd_soc_component *component)
{
	struct tas5806md_priv *tas5806md =
		snd_soc_component_get_drvdata(component);
	struct regmap *rm = tas5806md->regmap;
	...
	regmap_write(rm, REG_DEVICE_CTRL_2,
		(tas5806md->is_muted ? DCTRL2_MUTE : 0) |
		DCTRL2_MODE_PLAY);
	regmap_write(rm, REG_FAULT, ANALOG_FAULT_CLEAR);					// Is necessary for compatibility with TAS5828m
}

static const SNDRV_CTL_TLVD_DECLARE_DB_SCALE(tas5806md_vol_tlv, -10350, 50, 1);			// New (name, min, step, mute)

static const struct snd_kcontrol_new tas5806md_snd_controls[] = {				// New
	SOC_SINGLE_TLV	("Master Playback Volume", REG_VOL_CTL, 0, 255, 1, tas5806md_vol_tlv),	// (xname, reg, shift, max, invert, tlv_array)
};
...
/* The TAS5806MD DSP can't be configured until the I2S clock has been
 * present and stable for 5ms, or else it won't boot and we get no
 * sound.
 */
static int tas5806md_trigger(struct snd_pcm_substream *substream, int cmd,
			    struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:				// 4
		...
		if(tas5806md->dsp_cfg_data){
			send_cfg(rm, tas5806md->dsp_cfg_data,		// write firmware
				tas5806md->dsp_cfg_len);
			}
		else	{
			send_cfg(rm, dsp_cfg_firmware_missing,		// write minimal config
				ARRAY_SIZE(dsp_cfg_firmware_missing));
			}
		tas5806md->is_powered = true;
		tas5806md_refresh(component);
		break;
...
static const struct snd_soc_dapm_route tas5806md_audio_map[] = {
	{ "DAC", NULL, "DAC IN" },
	{ "OUT", NULL, "DAC" },
};

static const struct snd_soc_dapm_widget tas5806md_dapm_widgets[] = {
	SND_SOC_DAPM_AIF_IN("DAC IN", "Playback", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_DAC("DAC", NULL, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_OUTPUT("OUT")
};

static const struct snd_soc_component_driver soc_codec_dev_tas5806md = {
	.controls		= tas5806md_snd_controls,
	.num_controls		= ARRAY_SIZE(tas5806md_snd_controls),
	.dapm_widgets		= tas5806md_dapm_widgets,
	.num_dapm_widgets	= ARRAY_SIZE(tas5806md_dapm_widgets),
	.dapm_routes		= tas5806md_audio_map,
	.num_dapm_routes	= ARRAY_SIZE(tas5806md_audio_map),
	.use_pmdown_time	= 1,
	.endianness		= 1,
};

static int tas5806md_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	struct snd_soc_component *component = dai->component;
	struct tas5806md_priv *tas5806md =
		snd_soc_component_get_drvdata(component);
	...
	dev_dbg(component->dev, "set mute=%d (is_powered=%d)\n",
		mute, tas5806md->is_powered);
	tas5806md->is_muted = mute;
	if (tas5806md->is_powered)
		tas5806md_refresh(component);

	return 0;
}

static const struct snd_soc_dai_ops tas5806md_dai_ops = {
	.trigger		= tas5806md_trigger,
	.mute_stream		= tas5806md_mute,
	.no_capture_mute	= 1,
};

static struct snd_soc_dai_driver tas5806md_dai = {
	.name		= "tas5806md-amplifier",
	.playback	= {
		.stream_name	= "Playback",
		.channels_min	= 2,
		.channels_max	= 2,
		.rates		= TAS5806MD_RATES,
		.formats	= TAS5806MD_FORMATS,
	},
	.ops		= &tas5806md_dai_ops,
};

static const struct regmap_config tas5806md_regmap = {
	.reg_bits	= 8,
	.val_bits	= 8,
...
static int tas5806md_i2c_probe(struct i2c_client *i2c)
{
	...
	regmap = devm_regmap_init_i2c(i2c, &tas5806md_regmap);
	if (IS_ERR(regmap)) {
		ret = PTR_ERR(regmap);
	...
	tas5806md = devm_kzalloc(dev, sizeof(struct tas5806md_priv), GFP_KERNEL);
	if (!tas5806md)
		return -ENOMEM;

	tas5806md->pvdd = devm_regulator_get(dev, "pvdd");
	if (IS_ERR(tas5806md->pvdd)) {
		dev_err(dev, "failed to get pvdd supply: %ld\n",
			PTR_ERR(tas5806md->pvdd));
		return PTR_ERR(tas5806md->pvdd);
	}

	dev_set_drvdata(dev, tas5806md);
	tas5806md->regmap = regmap;
	tas5806md->gpio_pdn_n = devm_gpiod_get(dev, "pdn", GPIOD_OUT_LOW);
	if (IS_ERR(tas5806md->gpio_pdn_n)) {
		dev_err(dev, "error requesting PDN gpio: %ld\n",
			PTR_ERR(tas5806md->gpio_pdn_n));
		return PTR_ERR(tas5806md->gpio_pdn_n);
	}
	...
	snprintf(filename, sizeof(filename), "tas5806md_dsp_%s.bin",
		 config_name);
	...
	tas5806md->dsp_cfg_len = fw->size;
	tas5806md->dsp_cfg_data = devm_kmalloc(dev, fw->size, GFP_KERNEL);
	if (!tas5806md->dsp_cfg_data) {
		dev_err(dev, "firmware is not loaded, using minimal config\n");
		release_firmware(fw);
		goto err;
	}
	memcpy(tas5806md->dsp_cfg_data, fw->data, fw->size);
	release_firmware(fw);
	...
	ret = regulator_enable(tas5806md->pvdd);
	if (ret < 0) {
	...
	gpiod_set_value(tas5806md->gpio_pdn_n, 1);
	usleep_range(10000, 15000);

	/* Don't register through devm. We need to be able to unregister
	 * the component prior to deasserting PDN#
	 */
	ret = snd_soc_register_component(dev, &soc_codec_dev_tas5806md,
					 &tas5806md_dai, 1);
	if (ret < 0) {
		dev_err(dev, "unable to register codec: %d\n", ret);
		gpiod_set_value(tas5806md->gpio_pdn_n, 0);
		regulator_disable(tas5806md->pvdd);
		return ret;
	}
...
static void tas5806md_i2c_remove(struct i2c_client *i2c)
{
	struct device *dev = &i2c->dev;
	struct tas5806md_priv *tas5806md = dev_get_drvdata(dev);
...
static const struct i2c_device_id tas5806md_i2c_id[] = {
	{ "tas5806md", },
	{ }
};
MODULE_DEVICE_TABLE(i2c, tas5806md_i2c_id);

#if IS_ENABLED(CONFIG_OF)
static const struct of_device_id tas5806md_of_match[] = {
	{ .compatible = "ti,tas5806md", },
	{ }
};
MODULE_DEVICE_TABLE(of, tas5806md_of_match);
#endif

static struct i2c_driver tas5806md_i2c_driver = {
	.probe_new	= tas5806md_i2c_probe,
	.remove		= tas5806md_i2c_remove,
	.id_table	= tas5806md_i2c_id,
	.driver		= {
		.name		= "tas5806md",
		.of_match_table = of_match_ptr(tas5806md_of_match),
	},
};

module_i2c_driver(tas5806md_i2c_driver);

MODULE_AUTHOR("Andy Liu <andy-liu@ti.com>");
MODULE_AUTHOR("Daniel Beer <daniel.beer@igorinstitute.com>");
MODULE_AUTHOR("J.P. van Coolwijk <jpvc36@gmail.com>");
MODULE_AUTHOR("P. Steenbergen <p.steenbergen@openvoiceos.org>");
MODULE_DESCRIPTION("TAS5806MD Audio Amplifier Driver");
